...
class RandomRotate(object):
    ...
    def __call__(self, image, label):
        if random.random() < 0.5:
            ...
            scale = 1
            trans = cv2.getRotationMatrix2D(center, self.angle, scale)
            image = cv2.warpAffine(image, trans, (width,height),flags=cv2.INTER_CUBIC)
            label = cv2.warpAffine(label, trans, (width,height),flags=cv2.INTER_CUBIC)
        return [image, label]
